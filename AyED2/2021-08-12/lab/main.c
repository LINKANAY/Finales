...
void show_usage(const char *path)
{
  printf("Usage:\n");
  printf("%s [options]\n", path);
  printf("-f <files> test TAD War_Match on the all the files listed.\n");
...
int run_file(const char *filename, bool do_verify)
{
  ...
  if (do_verify)
  {
    length_fail = is_match_correct(match) ? (array_length == 0 ? length == 0 : length == array_length / 2) : length == 0;
  ...
  fails = size_fail + length_fail + correct + total_fail + array_fail;
  return (fails);
}
